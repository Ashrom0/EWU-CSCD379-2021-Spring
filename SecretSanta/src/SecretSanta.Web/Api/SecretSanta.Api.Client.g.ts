...
export interface IGroupsClient {
    ...
    assignment(id: number): Promise<void>;
}
...
export class GroupsClient implements IGroupsClient {
    ...
    assignment(id: number , cancelToken?: CancelToken | undefined): Promise<void> {
        let url_ = this.baseUrl + "/api/Groups/{id}/assignment";
        if (id === undefined || id === null)
            throw new Error("The parameter 'id' must be defined.");
        url_ = url_.replace("{id}", encodeURIComponent("" + id));
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.instance.request(options_).catch((_error: any) => {
            ...
            return this.processAssignment(_response);
        });
    }

    protected processAssignment(response: AxiosResponse): Promise<void> {
        const status = response.status;
        let _headers: any = {};
        if (response.headers && typeof response.headers === "object") {
            for (let k in response.headers) {
